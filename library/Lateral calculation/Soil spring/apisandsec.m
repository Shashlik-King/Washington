...
%                   AHA/LA 14.08.2007   Correcting - xtop = 0 => Kstop = 0
%                                       is introduced. Otherwise kstop can-
%                                       not be calculated. 
%                   AHA    22.08.2007   Lines 117 and 124 - the way the 
%                                       information is printed on screen is
%                                       changed.
%                   AHA    08.11.2007   Name of function is changed from
%                                       secspringstiff to apisandsec.
...
elseif xtop == 0;
	...
	if strcmp(loads.static_cyclic,'cyclic') % cyclic case
		kstop = Atop*putop/ytop*tanh(k*xtop*ytop/(0.9*putop));
	else
		kstop = Atop*putop/ytop*tanh(k*xtop*ytop/(Atop*putop));
	end
end
...
if ybot == 0;
	...
	if strcmp(loads.static_cyclic,'cyclic') % cyclic case
		ksbot = Abot*pubot/ybot*tanh(k*xbot*ybot/(0.9*pubot));
	else
		ksbot = Abot*pubot/ybot*tanh(k*xbot*ybot/(Abot*pubot));
	end
end