function [kttop ktbot] = kirschstiffclaytan(element,pile,loads,y_topbottom,i)
%--------------------------------------------------------------------------
% PURPOSE
...
for top_bottom = 1:2 %first the top node spring, thereafter the bottom node spring
    ...
    if y <= yini;
        kt(top_bottom) = kini;          
    elseif y <= ylim
        kt(top_bottom) = 0.25*pu/(ycmod*(16+9.6*log10(loads.n_cycles))^0.25*(y/ycmod)^0.75);% SPSO: Not entirely correct, since ycmod also depends on y
    else
        kt(top_bottom) = 0;
    end
end
