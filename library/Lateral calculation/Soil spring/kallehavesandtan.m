function [kttop ktbot] = kallehavesandtan(element,pile,loads,y_topbottom,i)
%--------------------------------------------------------------------------
% PURPOSE
...
if xtop == 0;
    ...
    if strcmp(loads.static_cyclic,'cyclic') % cyclic case
		kttop = Atop/0.9*k_mod_top*x0/(cosh(k_mod_top*x0*ytop/(0.9*putop)))^2;
	else
		kttop = k_mod_top*x0/(cosh(k_mod_top*x0*ytop/(Atop*putop)))^2;
	end
end
if strcmp(loads.static_cyclic,'cyclic') % cyclic case
	ktbot = Abot/0.9*k_mod_bot*x0/(cosh(k_mod_bot*x0*ybot/(0.9*pubot)))^2;
else
	ktbot = k_mod_bot*x0/(cosh(k_mod_bot*x0*ybot/(Abot*pubot)))^2;
end