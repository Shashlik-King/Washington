function [kstop ksbot] = stiffclay_w_o_water_sec(element,pile,loads,y_topbottom,i)
%--------------------------------------------------------------------------
% PURPOSE
...
for top_bottom = 1:2 %first the top node spring, thereafter the bottom node spring
    ...
    % Determination of secant stiffness in top of an element according to the API standard p.64.
    % -------- Static criterion -----------------------------------------------
    ...
    if y <= yini;
        ks(top_bottom) = kini; 
    elseif y <= ylim
        p = pu*(y/yc)^0.25/(16+9.6*log10(loads.n_cycles))^0.25;
        ks(top_bottom) = p/y;
    else
        p = pu;
        ks(top_bottom) = p/y;
