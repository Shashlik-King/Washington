...
% 2019.11.21    ASSV    Programming
    ...
    reduction_top=zeros(size(m.top{1,1},1),1); % reducing 1 from the last values of P,the value of p is reducing 1 by 1 to avoid repeatation
    reduction_top(:,1)=1;
    
    reduction_bot=zeros(size(m.top{1,1},1),1); % reducing 1 from the last values of P,the value of p is reducing 1 by 1 to avoid repeatation
    reduction_bot(:,1)=1;
    
    
    
    for z = 1:size(m.top{1,1},2) % FKMV size changed temporary fix
        
        p.top = m.top{1,z};
        p.bottom = m.bottom{1,z};
        y.top = teta.top{1,z};
        y.bottom = teta.bottom{1,z};
       % p_topvalue_total = m.top{2,z}(:,z);
       % p_botvalue_total = m.bottom{2,z}(:,z);
        ...
        for i =1:element.nelem-1
            ...
            value_y_top(1,1:size(y.top,2)) = y.top(i,:);
             ...
             if z<=size( m.top{i+1},2)
             p_topvalue_total = m.top{i+1,z}(1,z);
             
             else 
             p_topvalue_total = m.top{i+1,end}(1,end)-reduction_top(i,1);
             
             reduction_top(i,1)=reduction_top(i,1)+1;% increasing the recustion value for next z if any 
             end     
             
             
             if z<=size( m.bottom{i+1},2)  
             p_botvalue_total = m.bottom{i+1,z}(1,z);
             else 
             p_botvalue_total = m.bottom{i+1,end}(1,end)-reduction_bot(i,1);
             reduction_bot(i,1)=reduction_bot(i,1)+1;% increasing the recustion value for next z if any 
             end  
            ...
            if z>1 
                if i > 1
                    if strcmp(element.model_py(i-1),'Zero soil') % m.top{2,z}(i,z) ==  m.top{2,z}(i,z-1)
                        p_topvalue=z*10000;								  
                        p_botvalue=z*10000;
                    else
                        p_topvalue=p_topvalue_total;
                        p_botvalue=p_topvalue_total;
                    end
                else
                    if strcmp(element.model_py(i),'Zero soil') % m.top{2,z}(i,z) ==  m.top{2,z}(i,z-1)
                        p_topvalue=z*10000;								  
                        p_botvalue=z*10000;
                    else
                        p_topvalue=p_topvalue_total;
...
% % %             if i == 1
% % %                 p_av_mt = p_topvalue;
% % %                 p_botvalue_total_privious_node=p_botvalue;
% % %                 
% % %             elseif strcmp(element.model_py(i-1),'Zero soil')
% % %                 p_av_mt = p_topvalue;
% % %                 
% % %                 p_botvalue_total_privious_node=p_botvalue;
% % %             else
% % % 
% % %                 
% % %                 p_av_mt = (p_topvalue_total.*(abs(element.level(i,2))-...
% % %                     abs(element.level(i,1)))+p_botvalue_total_privious_node.*...
% % %                     (abs(element.level(i,1))-abs(element.level(i-1,1))))...
% % %                     ./((abs(element.level(i,2))-abs(element.level(i,1)))...
% % %                     +(abs(element.level(i,1))-abs(element.level(i-1,1)))); 
% % %                 
% % %                 p_botvalue_total_privious_node= p_botvalue;  % keep the value of top node for the futeare averaging 
% % % 
% % %             end
            ...
            % insert p values
            mysqlstr = [mysqlstr_ini,spring.col_p,''',',num2str(p_av_mt)];
            ...
            % y_top values
            mysqlstr = [mysqlstr_ini,spring.col_y,''',',num2str(p_av_mt)];
            for j = 1:20 
