...
ID_Soil_in_db='EW69_Ak_kirsch (2015)';   % over write the name 
...
data.revision.soil              = 1;              % revision no. of soil parameters to be used (1000 = latest revision)
...
settings.n_max                  = 500;        % number of load steps (lateral and axial)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
loads.static_cyclic             = 'static'; % 'cyclic' or 'static' in accordance with DNV OS-J10j of P_NGI
loads.n_cycles                  = 100;      % Number of cycles, relevant for Stiff clay w/o water only!
loads.A                         = 'P_NGI';    % use API or TUHH (Dührkop)  or  P_NGI   approach for determination of A, relevant for API/Kirsch sand
...
if plots.load_deflection == 1 || plots.load_deflection_axial == 1
	settings.max_load_ratio     = 50;       % attempts to apply a load equal to the one imported from Excel/database multiplied by this factor
end
%--------------------------------------------------------------------------
%% Output files
%--------------------------------------------------------------------------
