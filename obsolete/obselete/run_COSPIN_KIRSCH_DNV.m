...
% ID ={'L0GW01_ULS_BE_ver3'}; % API Loads received 2020-10-29
Water_Depth='DEEP'    %'DEEP' or   'Shallow'
ID ={'EW1-69'}; % API Loads received 2020-11-06
loadtype='Robustness 500-year DEEP';

[loads, names, DLC] = extractloads_fkmv(Water_Depth);
...
%% Project data
%--------------------------------------------------------------------------
data.project                    = 'Vietnam Banh Dai OWF';   % 'Project name'
data.A_number                   = 'A205414';   % 'Project number'
data.location                   = ID{loc};   % 'Project location'
data.db_location                = strcat('L3',ID{loc});
data.db_location_geo            = strcat('L',ID{loc}); %'Database location to read data
data.prepared_by                = 'KAGP';   %  Initials of preparer
data.id                         = strcat('L3',ID{loc}); % Id to store into the database   
data.revision.global            = 1000;   % global revision no. for selected location to be used,
...
soil.psf_value                  = 1.4;            % undrained psf applied to clay su directly
soil.type                       = 'BE';
...
data.revision.output            = 61;             % revision no. for storing results into the database  
...
%--------------------------------------------------------------------------
settings.nelem_factor           = 5;        % [m-1] minimum number of pile beam elements per meter 2.2
...
settings.analysis_type          = 1;        % 0 = normal axial assessment, 1 = settlements
...
settings.save_plots             = 1;        % save plots in output folder (1 = yes, 0 =no)
...
elseif strcmp(settings.database,'Manual')
    [scour soil pile loads settings] = manual_data_input_excel(pile,data,soil,...
        loads,settings,SF); % load soil and pile information from Exce-file
end
...
element.psf_switch = soil.psf;
element.psf_undrained = soil.psf_value;
%--------------------------------------------------------------------------
