...
% ID ={'A-22_alt_crit_seismic'}; % API
ID ={'A-22_crit_seismic'}; % API
ID ={'A-22_seismic'}; % API

ID ={'A-25_crit_seismic'}; % API
% ID ={'A-25_seismic'}; % API
for loc = 1:length(ID)
%% Project data
%--------------------------------------------------------------------------
...
plots.pilehead_vs_length        = 1;        % 1 = yes, 0 = no
...
settings.lateralmultipliers 	= 1; 		% account for p and y multipliers: 0-> not accounted 1-> accounted
...
if loads.n_cycles==10000 && plots.permanent_rot_def == 1 && iter_global == 1
    loads.H=loads.H*0.3;
    loads.M=loads.M*0.3;
elseif settings.seismic == 1 && iter_global == 1
    loads.H=loads.H*0.4;
    loads.M=loads.M*0.4;
end
% if loads.n_cycles==10000 && plots.permanent_rot_def == 1 && iter_global == 1
